...
public class HexagonMapPanel extends JPanel {
    ...
    public void drawCharacters(Graphics g) {
        for (int i = 0; i < m.characters.size(); i++) {
            File file = new File(m.characters.get(i).draw());
            try {
                BufferedImage image = ImageIO.read(file);
               g.drawImage(image, m.characters.get(i).currentField.p.xpoints[0], m.characters.get(i).currentField.p.ypoints[0], this);
            } catch (IOException e) {
